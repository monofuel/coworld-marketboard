...
This is a **Bitworld Coworld game** (`marketboard`).

- Read [README.md](README.md) first for the core concept, economy rules, roles (gatherer/crafter), market mechanics, scoring, signaling, legends mode, and future v2/v3 ideas (danger, permadeath, etc.).
- This project builds on the `bitworld` engine (protocol, server, sprite rendering, replays). See the [Supporting Repositories](#supporting-repositories) section below.
- Follow the Nim style guide in this file. Keep changes minimal, deterministic, and let errors bubble up naturally.

## Supporting Repositories

This project is part of the larger Coworld / Metta ecosystem. Key related repositories (all siblings under `../` from this project):

- **`../bitworld`** — Core game engine, sprite protocol (`bitworld/protocol`), server (`bitworld/server`), replay format, legends mode, cogame runtime. This marketboard game is built *on top of* bitworld. See `nimby.lock`, `nim.cfg`, and imports throughout `src/`.
