...
- **Palette fix (bright red ground → eye-searing neon green)**: Copied canonical `pallete.png` from `../bitworld/client/data/` into `data/pallete.png`. Added real loader in `loadRenderAssets` (sim.nim:1251) that forces `Palette[]` from local PNG. Final terrain: grass on 10 (0,135,81 deeper teal — easy on the eyes), path 5, wall 12. Selection highlight moved to 3 (bright red accent) so it pops on grass instead of vanishing. Progress bar fill in player view uses the freed 11. Wood/hardwood etc. remapped for coherence. Ground no longer red or neon.
