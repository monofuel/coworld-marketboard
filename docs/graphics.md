...
- **Typography**: Already pulled good `readTiny5Font` from bitworld/pixelfonts. Upgraded spectator UI (global_render.nim) to use `renderTextToRgba` (tiny5) consistently for legends + debug tick counter. Removed blocky font dep for clean global view. Simplified `buildLegendPacket` (no more maxChars/letterSprites checks). Legend text now uses palette 8 (yellow pop) for contrast.
...
- **UI atlas + scoreboard/legend polish**: Synced `shell.png` + `dpad.png` from bitworld atlas into `data/atlas/` (ready for icons without bloat). Scoreboard already uses tiny5 + swatches (height was pre-fixed at 256). Legends now tighter rows (8px), consistent font, brighter color. Overall cleaner hierarchy/contrast for lobby TV.
- **Other**: `make check` passes clean. All changes minimal, follow Nim style (## docs inside procs, grouped consts, sibling source reliance, no unrelated refactors). Deleted unnecessary blocky usage in spectator path.
